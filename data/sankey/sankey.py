...
) + DF_SANKEY["LinksMSN"]["MSN"].drop_duplicates().values.tolist()
...
for index, value in DF_SANKEY["LinksMSN"].iterrows():
    DF_LINKS[value["Link"]] += DF_DATASET[value["MSN"]
                                          ].transform(lambda x: _temp(x, value["Coefficient"]))
...
for index, value in DF_SANKEY["GroupSum"].iterrows():
    parent = value["Parent"]
    child = value["Child"]
    CHILDS[parent] = [_value["Child"] for index,
                      _value in DF_SANKEY["GroupSum"].iterrows() if (_value["Parent"] == parent)]
...
for group in GROUPMAP.keys():
    ...
    for source in GROUPMAP[group]["sourceLinks"].keys():
        DF_LINKS[ENCODE_LINK_ID(
            source, group)] = DF_LINKS[GROUPMAP[group]["sourceLinks"][source]].sum(axis=1)
        LINKS[ENCODE_LINK_ID(source, group)] = {
            "Source": source,
            "Target": group
        }
        DF_LINKS = DF_LINKS.drop(
            columns=GROUPMAP[group]["sourceLinks"][source])
    for target in GROUPMAP[group]["targetLinks"].keys():
        DF_LINKS[ENCODE_LINK_ID(
            group, target)] = DF_LINKS[GROUPMAP[group]["targetLinks"][target]].sum(axis=1)
        LINKS[ENCODE_LINK_ID(group, target)] = {
            "Source": group,
            "Target": target
        }
        DF_LINKS = DF_LINKS.drop(
            columns=GROUPMAP[group]["targetLinks"][target])
...
DF_NODES = pd.melt(frame=DF_NODES, id_vars=[
                   "StateCode", "Year"], var_name="Node", value_name="Data")
DF_LINKS = pd.melt(frame=DF_LINKS, id_vars=[
                   "StateCode", "Year"], var_name="Link", value_name="Data")
...
# transform to dict
DATA_NODES = DF_NODES.set_index(
    keys=["StateCode", "Year", "Node"]).to_dict(orient="index")
DATA_LINKS = DF_LINKS.set_index(
    keys=["StateCode", "Year", "Link"]).to_dict(orient="index")
...
for state, year in STATE_YEARS:
    ...
    for node in nodelist:
        ...
        DATA[state][year]["nodes"].append(obj)
...
# write data to file
open("transformedsankey.json", "w").write(
    json.dumps(DATA, sort_keys=False, indent='\t'))

# test
open("sankey.json", "w").write(json.dumps(
    DATA[NEEDED_STATE][NEEDED_YEAR], sort_keys=False, indent='\t'))

# DF_DATASET = pd.read_csv(FILE_DATASET, usecols=[
#                          "StateCode", "Year", "MSN", "Data"])

# DF_DATASET = DF_DATASET[(DF_DATASET["StateCode"] == NEEDED_STATE) & (
#     DF_DATASET["Year"] == NEEDED_YEAR)]
# #  & (DF_DATASET["MSN"].isin(NEEDED_MSN))]
